...
class Bdrmapit:
    ...
    #     # Use heuristics to determine link votes
    #     vtype = None
    #     vote_weights = Counter(interface.asn for interface in router.interfaces)
    #     succs = Counter()
    #     nonvrf = Counter()
    #     sasn_origins = defaultdict(set)
    #     for edge in router.succ:
    ...
    #             if not edge.node.vrf:
    #                 nonvrf[succ_asn] += 1
    #             vote_weights[succ_asn] += len(edge.node.succ)
    #     if DEBUG:
    #         print('Succs: {}'.format(succs))
    #         print('VType: {}'.format(vtype))
    # 
    #     # Create votes counter and add interface AS
    #     votes = succs + iasns
    #     # for asn, num in list(votes.items()):
    #     #     votes[asn] *= vote_weights[asn]
    #     if DEBUG: print('Votes: {}'.format(votes))
    #     if not votes:
    #         return -1, -1
    # 
    ...
    #     else:
    #         if nonvrf:
    #             if DEBUG: print('Non VRF: {}'.format(nonvrf))
    #             asn = max(asns, key=lambda x: (nonvrf[x], -self.bgp.conesize[x], x))
    #             # utype += VOTE_TIE
    #             utype += 50000
    #         else:
    ...
    #         #     asn = min(asns, key=lambda x: (self.bgp.conesize[x], -x))
    #             if DEBUG:
    #                 print('Vote Weights: {}'.format(vote_weights))
    #             # asn = min(asns, key=lambda x: (-vote_weights[x], self.bgp.conesize[x], -x))
    #             utype += VOTE_TIE
    #     return asn, utype
    ...
    def annotate_router(self, router: Router):
        ...
        if len(asns) == 1:
            ...
            if len(router.succ) == 1:
                ...
                if sasn in succs and (any(sasn == iasn or sasn in self.bgp.peers[iasn] for iasn in iasns) or len(isucc.pred) > 1):
                    asn = sasn
                    utype += 5000000
                    # return sasn, 5000000 + utype
            if not asn:
                if DEBUG: print('Conesizes: {}'.format({a: self.bgp.conesize[a] for a in asns}))
                # asn = min(asns, key=lambda x: (self.bgp.conesize[x], -x))
