...
def hidden_asn(bdrmapit: Bdrmapit, iasns, asn, utype, votes: TCounter[int]):
...
def annotate_router(bdrmapit: Bdrmapit, router: Router, rupdates: Updates, iupdates: Updates):
    ...
    if len(succs) == 1 or len({bdrmapit.as2org[sasn] for sasn in succs}) == 1:
        ...
        if succs[sasn] > sum(iasns.values()) / 4:
            ...
            if not any(bdrmapit.bgp.rel(iasn, sasn) for iasn in succ_origins[sasn]) and any(
                ...
                return hidden_asn(bdrmapit, Counter(succ_origins[sasn]), sasn, utype, iasns)
            for isucc in edges:
    ...
    if check_hidden:
        ...
        if len(intersection) == 1:
            interasn = peek(intersection)
            interorg = bdrmapit.as2org[interasn]
            if DEBUG:
                print('Intersection Org {} in {}'.format(interorg, {bdrmapit.as2org[vasn] for vasn in votes}))
            if interorg not in {bdrmapit.as2org[vasn] for vasn in votes}:
                asns = [interasn]
                utype += 10000
    if len(asns) == 1:
        asn = asns[0]
    ...
    if iasns and asn not in iasns and not any(bdrmapit.bgp.rel(iasn, asn) for iasn in iasns):
        return hidden_asn(bdrmapit, iasns, asn, utype, votes)
    return asn, utype
